...
Building a microservices architecture implies utilizing a distributed system[^3]. This is _not_ an implication to be taken lightly. The foundation of a distributed system has several key fallacies that must be understood in order to build a reliable system!
...
When a system is a traditional monolith, the external service is just another part of the monolith so you’re just making a function call, not a network call; if you’re able to make the request, they’re able to respond. Failure modes are more limited.
...
Although it takes time to make a function call, it’s so minimal that for most purposes it can be considered zero latency. A network call, on the other hand, will take a nontrivial amount of time (maybe 2 ms).
...
> Network topology is the arrangement of the elements (links, nodes, etc.) of a communication network.[^5]
...
Working with distributed systems requires a change of mindset. Most of us learned how to program by writing simple command-line applications. Perhaps your first program was the iconic “Hello, world.” Then you progressed to programs that could read in a file, make some changes, and spit out another file. With time, you developed a mental model of how code executes with the simplified assumptions that can be made in a **single computer environment**.

Once a system of computers must solve a problem _in a coordinated manner_, the assumptions we previously could make turn into misconceptions. Communicating with another computer is fundamentally different; we must retrain our mental models to account for this.

## Challenges of a microservices architecture

Here are the primary challenges related to microservices I have experienced in practice.
...
It's true that the codebases of a microservices are easier to fully understand. It's possible for a single engineer to keep all the details in his or her head, leading to safer changes. That said, deployments sometimes must be coordinated and it's easy to make mistakes across teams.

Imagine that service A owned by one team wants to expose a new API that will be used by another team's service B. The fact that service A's deployment had to be rolled back is not communicated to the other team and they proceed with their release, causing all requests to fail which rely on the new APIs. Putting a strong emphasis on backwards compatibility can help these situations. In a monolithic architecture, the deployment was all or nothing, so either the new APIs were released _and_ used, or neither.
...
This blog post has admittedly had a cautionary tone. We covered the fallacies of distributed systems and many of the challenges of an MSA. What are the benefits of microservices that have motivated its adoption?

### Conway's Law

A monolith makes sense for a company with a small engineering team. Once the number of developers increases, however, the monolith becomes unwieldy, difficult (or impossible) to fully understand, and diffused in ownership. Conway's Law simply states that a company's architecture will mimic the way teams are structured. So as new engineers are hired and teams split or are moved to another office, the architecture naturally evolves as well. Sometimes monoliths refute this evolution and fight against Conway's Law.
...
In conclusion, I do not believe that we as an industry are making a huge mistake in adopting microservices. That said, while microservices clearly solve critical problems, they present their own challenges and are not a silver bullet by any means. Best practices like service meshes and gateways help mitigate the weaker points of microservices leaving us in a more scalable position.

## Further Reading

* Wikipedia:
* 10 books for new software engineers
...
[^6]: https://en.wikipedia.org/wiki/Exponential_backoff
